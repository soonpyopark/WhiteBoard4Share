...
export function TableEditorOverlay({
  ...
  const existing = session.id ? engine.getTableObject(session.id) : null;
  ...
  const borderColor = existing?.borderColor ?? settings.borderColor;
  const defaultColWidth = existing?.cellWidth ?? settings.cellWidth;
  const defaultRowHeight = existing?.cellHeight ?? settings.cellHeight;
  const scale = engine.getViewScale();
  const screen = engine.worldToScreen(session.topLeftX, session.topLeftY);
  const toScreenPx = (value: number) => Math.max(1, Math.round(value * scale));
  const scaledGutter = toScreenPx(GUTTER_SIZE);
  const scaledHeader = toScreenPx(HEADER_HEIGHT);
  const scaledColWidths = session.colWidths.map((width) => toScreenPx(width));
  const scaledRowHeights = session.rowHeights.map((height) => toScreenPx(height));

  const updateSessionLayout = (layout: TableLayoutState, activeRow = session.activeRow, activeCol = session.activeCol) => {
    onSessionChange(applyLayoutToSession(session, layout, activeRow, activeCol));
  };
  ...
  const handlePanelBlur = useCallback(
    (event: React.FocusEvent<HTMLDivElement>) => {
      ...
      if (optionsOpen || contextMenu) return;
      const next = event.relatedTarget as Node | null;
      if (next && panelRef.current?.contains(next)) return;
      onCommit(session);
    },
    [contextMenu, onCommit, optionsOpen, session],
  );
  ...
  const openRowMenu = (index: number, event: React.MouseEvent | React.PointerEvent) => {
    event.preventDefault();
  ...
  const openColMenu = (index: number, event: React.MouseEvent | React.PointerEvent) => {
    event.preventDefault();
    event.stopPropagation();
    suppressBlurUntilRef.current = performance.now() + 300;
  ...
  const runStructureAction = (action: () => TableLayoutState, activeRow?: number, activeCol?: number) => {
    updateSessionLayout(action(), activeRow ?? session.activeRow, activeCol ?? session.activeCol);
    setContextMenu(null);
  };
  ...
  const startColResize = (colIndex: number, event: React.PointerEvent<HTMLDivElement>) => {
    event.preventDefault();
    event.stopPropagation();
    suppressBlurUntilRef.current = performance.now() + 600;
      ...
      items.push(
        ...
        },
        {
          ...
          onClick: () => {
            ...
            runStructureAction(() => deleteRowAt(layoutFromSession(session), rowIndex));
          },
        },
      );
    }
      ...
      items.push(
        ...
        },
        {
          ...
          onClick: () => {
            ...
            runStructureAction(() => deleteColAt(layoutFromSession(session), colIndex));
          },
        },
      );
    }
  ...
  return (
    <>
      <div
        ...
        style={{
          left: `${screen.x - frameInset - GUTTER_SIZE * scale}px`,
          top: `${screen.y - frameInset - HEADER_HEIGHT * scale}px`,
        }}
        onPointerDown={(event) => event.stopPropagation()}
      ...
      >
        <div className="canvas-table-editor__toolbar">
          <button
            type="button"
            disabled={session.rows >= TABLE_MAX_ROWS}
            onClick={() => updateSessionLayout(appendRow(layoutFromSession(session), defaultRowHeight))}
          >
            행+
          </button>
          <button
            type="button"
            disabled={session.rows <= 1}
            onClick={() => updateSessionLayout(removeLastRow(layoutFromSession(session)))}
          >
            행-
          </button>
          <button
            type="button"
            disabled={session.cols >= TABLE_MAX_COLS}
            onClick={() => updateSessionLayout(appendCol(layoutFromSession(session), defaultColWidth))}
          >
            열+
          </button>
          <button
            type="button"
            disabled={session.cols <= 1}
            onClick={() => updateSessionLayout(removeLastCol(layoutFromSession(session)))}
          >
            열-
          </button>
        ...
        <div className="canvas-table-editor">
          <div
            ...
            style={{
              width: `${scaledGutter + scaledColWidths.reduce((total, width) => total + width, 0)}px`,
              height: `${scaledHeader + scaledRowHeights.reduce((total, height) => total + height, 0)}px`,
            }}
          >
            <table
                  ...
                  {session.colWidths.map((colWidth, colIndex) => (
                    ...
                    >
                      <button
                        ...
                        onClick={(event) => openColMenu(colIndex, event)}
                ...
                {session.cells.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    <th
                      ...
                      style={{
                        width: `${scaledGutter}px`,
                        height: `${scaledRowHeights[rowIndex] ?? toScreenPx(session.rowHeights[rowIndex] ?? defaultRowHeight)}px`,
                      }}
                    >
                      <button
                        type="button"
                        className="canvas-table-editor__header-btn"
                        onClick={(event) => openRowMenu(rowIndex, event)}
